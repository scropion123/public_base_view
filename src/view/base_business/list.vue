...
import {
	getBusinessList, getProvinceCityList,
} from "@/api/api";
export default {
	components: {
		pageTitle
	...
	created() {
		let msg = storage.getItem("business-list-msg");
		...
		if (msg) {
			...
			storage.removeItem("business-list-msg");
		}
	...
	methods: {
		saveFilter() {
			...
			storage.setItem("business-list-msg", msg);
		},
		detail(r) {
			this.saveFilter();
			this.$router.push("/base/business/detail/" + r.id);
		},
		edit(r) {
			this.saveFilter();
			this.$router.push("/base/business/add/" + r.id);
		},
		// 分页
